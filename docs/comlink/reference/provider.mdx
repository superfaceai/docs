...
- `statusCode` (number, optional) - HTTP status code that should be returned from initial call (containing challenge header). Default value is `401`.
- `challengeHeader` (string, optional) - Case insensitive name of header containing challenge from the server. Default value is `www-authenticate`.
- `authorizationHeader` (string, optional) - Case insensitive name of header containing authorization. Default value is `Authorization`.
