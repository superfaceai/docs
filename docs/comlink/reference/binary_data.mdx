...
Use `BinaryData` class in OneSDK to upload content by streaming bytes.
...
If `BinaryData` instance is used as body or value in `FormData`, the content is streamed. This allows you to upload and work with big files.
...
Comlink maps right now doesn't support Script expression as statement. The only workaround for now is to use variable assignement.
