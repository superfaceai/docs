---
slug: /comlink/references/profile
---

# Comlink Profile Reference

Comlink Profile is a language for describing business capabilities as use-cases apart from implementation details.
...
* `safe` - The use-case doesn't result in server-side changes, so the client can retry the use-case when there's a failure
...
This is informational for the tooling to know how it should handle retrying failures.

:::info HTTP Methods

If you are familiar with REST APIs and HTTP methods, you can think of safety in this manner:

- `safe` corresponds to `GET` and `HEAD` methods
- `unsafe` corresponds to the `POST` method
...
This example shows a title of "Retreive Shipment Status" and a description of "Get the current shipment status" using triple double-quotes.
...
* Fields marked with exclamation mark are required as in `trackingNumber! string`
...
Comlink has support for primatives that can be used as field specifications.
