...
Superface expects an authorization token for Resend. You can add this as `RESEND_TOKEN` in your environment or create an `.env` file in the `superface` folder that the CLI created. To learn more how to handle API credentials, see [Setting provider API keys](../guides/api-keys.md).
