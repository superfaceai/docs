...
In order to list the organizations correctly, you will need to modify the boilerplate code from the Pipedrive documentation so it contains a real request object. From the `superface` folder, open `organizations-management.get-collection.pipedrive.mjs` and modify the object that starts on line 23.
