...
This configuration can be passed as a `super.json` file (located by default in the `superface` folder under your project root) or as a full object (if, for instance, your environment makes it inconvenient to use the filesystem). For example:

```javascript
// If it exists, `super.json` is loaded from `superface/super.json` under project root
const sdk = new SuperfaceClient();

// `super.json` is loaded from elsewhere on the filesystem
const sdk = new SuperfaceClient({superJson: "./my_vendor_stuff/superface_config.json"});
...
First, **install** a use case profile using the [Superface CLI](https://github.com/superfaceai/cli).
...
## Locally stored maps, profiles and providers
