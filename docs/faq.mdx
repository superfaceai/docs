...
Superface's OneSDK is currently in beta, so you should expect that changes will be made to future versions. We recommend that you test your application with any new version of OneSDK that you upgrade to before releasing to production.
...
New releases and changelog information can be found in the [Announcements](https://github.com/orgs/superfaceai/discussions/categories/announcements) section of GitHub Discussions. You can also check for updates by running `brew update` and looking for updates to the `superface/cli` tap.
