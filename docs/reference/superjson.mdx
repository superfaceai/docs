...
`super.json` is the main Superface configuration file, located by default in the `superface` folder under your project root. It specifies which profiles, maps, and providers are used, and their configuration (e.g. API credentials).

It is not required (in most cases) in order to use Superface, but you may still find it useful as your project grows in size and complexity, as it provides a central location for configuring details concerning your API integrations.

The two situations in which `super.json` is required are:
