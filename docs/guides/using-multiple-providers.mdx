...
We recommend you to read through the [Getting&nbsp;started](../introduction/getting-startedmdx) first if you haven’t done so.
...
async function run() {
  ...
  const provider = await sdk.getProvider('mailgun');
...
```js
const result = await profile.getUseCase('UserRepos').perform(inputs, provider);
```
